...
router.post('/GPReg', async (req, res) =>{
    ...
    try {
        const gpEmail = new GPRegistration(req.body);
        let emailExist = await GPRegistration.findOne({email:req.body.email});
        if (emailExist) {
            return res.status(400).send("Sorry this email is already taken");
		} else {
			await GPRegistration.register(gpEmail, req.body.password, (error) => {
				if (error) {
					throw error;
				}
